...
public class ActTaskServiceImpl implements IActTaskService{
    ...
    @Override
    public void ProjectApproval(ApplyProject applyProject, String taskId) throws Exception {
        //进行任务审批
        try
        {
            taskService.complete(taskId);
        ...
        applyProject = applyProjectMapper.selectByPrimaryKey(applyProject.getId());
        ...
        Integer isAgree = applyProject.getStatus();


        if (isAgree == CsEnum.ApplyRoomForm.APPLY_STATUS_AGREE.getValue())
        {
            // 设为使用中 描述 房间状态情况 0：空闲，1：预约中 2：使用中，3停用
            project.setStatus(CsEnum.project.PROJECT_STATUS_APPLYED.getValue());
        ...
        {
            // 不同意 设为0
            project.setStatus(CsEnum.project.PROJECT_STATUS_FREE.getValue());
        }
